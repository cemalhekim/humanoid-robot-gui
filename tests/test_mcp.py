...
class McpRequestTest(unittest.TestCase):
    ...
    def test_tools_list_matches_chat_tools(self) -> None:
        # The pose tools are static now: propose_arm_pose + move always ride
        # along when the move tool is enabled, independent of saved recordings.
        ...
        self.assertEqual(
            ...
            + ["propose_arm_pose", "move"],
        )

    def test_move_descriptor_offers_only_proposed_and_home(self) -> None:
        tools = self.store.mcp_request(rpc("tools/list"))["result"]["tools"]
        move = next(t for t in tools if t["name"] == "move")
        self.assertEqual(move["inputSchema"]["properties"]["position"]["enum"], ["proposed", "home"])
